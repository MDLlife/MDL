...
function checkHw(wait) {
  ...
  checkHwSubscription = Observable.of(1)
    ...
    .subscribe(
      () => {
        const connected = HID.devices().find((d) => d.manufacturer === "MDLFoundation");
...
ipcMain.on('hwGetDeviceConnectedSync', (event) => {
  event.returnValue = HID.devices().find((d) => d.manufacturer && d.manufacturer === "MDLFoundation") !== undefined;
  checkHw(false);
});
...
ipcMain.on('hwSignMessage', (event, requestId, addressIndex, message) => {
  const promise = deviceWallet.devMDLSignMessage(addressIndex, message, pinEvent);
  promise.then(
