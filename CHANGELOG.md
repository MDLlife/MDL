...
- `/api/v1/explorer/address` endpoint (use `GET /api/v1/transactions?verbose=1` instead). See https://github.com/MDLlife/MDL/blob/develop/src/api/README.md#migrating-from--api-v1-explorer-address
- The unversioned REST API (the `-enable-unversioned-api` is removed, prefix your API requests with `/api/v1` if they don't have an `/api/vx` prefix already). See https://github.com/MDLlife/MDL/blob/develop/src/api/README.md#migrating-from-the-unversioned-api
...
- `/api/v1/wallet/spend` endpoint (use `POST /api/v1/wallet/transaction` followed by `POST /api/v1/injectTransaction` instead). See https://github.com/MDLlife/MDL/blob/develop/src/api/README.md#migrating-from--api-v1-spend
...
- `/api/v1/health` will return correct build info when running Docker containers based on `MDLlife/MDL` mainnet image.
- #2083, Windows desktop wallet sometimes shows "Error#1" on start

### Changed

- Extend URI specification to allow plain addresses (i.e. without a `MDL:` prefix)
- Switch `mdl-cli` from `urfave/cli` to `spf13/cobra`.
...
- Remove libmdl source code. Migrated to https://github.com/MDLlife/libmdl
...
- JSON-RPC 2.0 interface (this is no longer used by the CLI tool, and the REST API supports everything the JSON-RPC 2.0 API does). See https://github.com/MDLlife/MDL/blob/develop/src/api/README.md#migrating-from-the-jsonrpc-api
- `/api/v1/wallet/spend` endpoint (use `POST /api/v1/wallet/transaction` followed by `POST /api/v1/injectTransaction` instead). See https://github.com/MDLlife/MDL/blob/develop/src/api/README.md#migrating-from--api-v1-spend
- The unversioned REST API (the `-enable-unversioned-api` option will be removed, prefix your API requests with `/api/v1`). See https://github.com/MDLlife/MDL/blob/develop/src/api/README.md#migrating-from-the-unversioned-api
- `/api/v1/explorer/address` endpoint (use `GET /api/v1/transactions?verbose=1` instead). See https://github.com/MDLlife/MDL/blob/develop/src/api/README.md#migrating-from--api-v1-explorer-address
...
- Database verification will only be performed once when upgrading to the next version. Verification will not be performed on subsequent upgrades unless necessary. To force verification, use `-verify-db=true`. Note that it is unsafe to downgrade the MDL node without erasing the database first.
...
- Daemon configured builds will be available on the [releases](https://github.com/MDLlife/MDL/releases) page. The builds available for previous versions are configured for desktop client use.
- `mdl-cli` builds will be available on the [releases](https://github.com/MDLlife/MDL/releases) page.
...
- Complete support for `cipher` package in `libmdl` C API.
- Add `coin`, `wallet`, `util/droplet` and `util/fee` methods as part of `libmdl` C API
- Add `make update-golden-files` to `Makefile`
- Add CLI `richlist` command
- Add `util/droplet` and `util/fee` API's as part of `libmdl`
...
- `go run cmd/MDLlife/MDL.go` will have exit status 1 on failure and exit status 2 on panic
...
- Add Dockerfile in `docker/images/dev-dind` to build a docker in docker image based on mdldev-cli.
...
- Write [specification for mdl URIs](https://github.com/MDLlife/MDL#uri-specification) (based upon bip21)
...
- Add `/logs` api to filter mdl logs, so that we can add a debug panel to the GUI wallet to show logs
...
- CLI's `walletHistory` command prints amounts as fixed-point decimal strings. Previously, it printed amounts as integers representing whole mdl amounts, and did not support droplets / fractional mdls.
...
[Unreleased]: https://github.com/MDLlife/MDL/compare/master...develop
[0.25.1]: https://github.com/MDLlife/MDL/compare/v0.25.0...v0.25.1
[0.25.0]: https://github.com/MDLlife/MDL/compare/v0.24.1...v0.25.0
[0.24.1]: https://github.com/MDLlife/MDL/compare/v0.24.0...v0.24.1
[0.24.0]: https://github.com/MDLlife/MDL/compare/v0.23.0...v0.24.0
[0.23.0]: https://github.com/MDLlife/MDL/compare/v0.22.0...v0.23.0
[0.22.0]: https://github.com/MDLlife/MDL/compare/v0.21.1...v0.22.0
[0.21.1]: https://github.com/MDLlife/MDL/compare/v0.21.0...v0.21.1
[0.21.0]: https://github.com/MDLlife/MDL/compare/v0.20.4...v0.21.0
[0.20.4]: https://github.com/MDLlife/MDL/compare/v0.20.3...v0.20.4
[0.20.3]: https://github.com/MDLlife/MDL/compare/v0.20.2...v0.20.3
[0.20.2]: https://github.com/MDLlife/MDL/compare/v0.20.1...v0.20.2
[0.20.1]: https://github.com/MDLlife/MDL/compare/v0.20.0...v0.20.1
[0.20.0]: https://github.com/MDLlife/MDL/compare/v0.19.1...v0.20.0
[0.19.1]: https://github.com/MDLlife/MDL/compare/v0.19.0...v0.19.1
[0.19.0]: https://github.com/MDLlife/MDL/commit/dd924e1f2de8fab945e05b3245dbeabf267f2910
