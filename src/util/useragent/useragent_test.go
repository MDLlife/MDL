...
func TestDataBuild(t *testing.T) {
	cases := []struct {
		...
		{
			name:      "with remark",
			userAgent: "MDL:0.24.1(remark; foo)",
			data: Data{
				Coin:    "MDL",
				Version: "0.24.1",
				Remark:  "remark; foo",
...
func TestParse(t *testing.T) {
	cases := []struct {
		...
		{
			name:      "too long",
			userAgent: fmt.Sprintf("mdl:0.24.1[abcdefg](%s)", strings.Repeat("a", 245)),
			err:       ErrTooLong,
		},
		{
...
func TestDataJSON(t *testing.T) {
	...
	x, err = json.Marshal(d)
	require.NoError(t, err)
	require.Equal(t, `"mdl:0.25.0"`, string(x))
	...
	x, err = json.Marshal(d)
	require.NoError(t, err)
	require.Equal(t, `"mdl:0.25.0(foo; bar)"`, string(x))
	...
	err = json.Unmarshal([]byte(`"mdl:0.25.0(<>)"`), &e)
...
func TestSanitize(t *testing.T) {
	...
	x := "MDL:0.25.0(foo; bar)"
	require.Equal(t, x, Sanitize(x))

	// Should not have anything stripped
	x = "MDL:0.25.1(foo; bar)"
	require.Equal(t, x, Sanitize(x))
}
