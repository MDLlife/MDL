...
A REST API implemented in Go is available, see [MDL REST API Client Godoc](https://godoc.org/github.com/MDLlife/MDL/src/api#Client).
...
* `PROMETHEUS` - This is the `/api/v2/metrics` method exposing in Prometheus text format the default metrics for MDL node application
...
Parses and validates a MDL address. Returns the address version in the response.
...
```json
{
    "address": "/Users/user/.mdl/wallets"
}
```
...
See [`coin.Transaction.Serialize`](https://godoc.org/github.com/MDLlife/MDL/src/coin#Transaction.Serialize).
...
You must use a string for `"coins"` instead of an integer measured in "droplets" (the smallest unit of currency in MDL, 1/1000000 of a mdl).
For example, if you sent 1 MDL with `/api/v1/spend` you would have specified the `coins` field as `1000000`.
