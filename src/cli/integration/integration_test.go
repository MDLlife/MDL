...
var (
	...
	stripCoverageReport = regexp.MustCompile(`PASS\ncoverage: [\d\.]+% of statements in github.com/MDLlife/MDL/\.\.\.\n$`)
)
...
func execCommandCombinedOutput(args ...string) ([]byte, error) {
	...
	// PASS
	// coverage: 8.1% of statements in github.com/MDLlife/MDL/...
...
func TestMain(m *testing.M) {
	...
	args := []string{"test", "-c", "-tags", "testrunmain", "-o", binaryPath, "-coverpkg=github.com/MDLlife/MDL/...", "../../../cmd/mdl-cli/"}
	...
	tt := []struct {
	...
	}{
		{
			...
			check: func(t *testing.T, v []byte) {
				...
				require.Equal(t, wallet.CoinTypeMDL, wallet.CoinType(w.Meta["coin"]))
